...
def test_all_users(test_app, monkeypatch):
    ...
    monkeypatch.setattr(src.api.users.users, "get_all_users", mock_get_all_users)
    client = test_app.test_client()
...
def test_remove_user(test_app, monkeypatch):
    # Why AttrDict instead of simple dict?
    # Ans: To enable attr-like access e.g. user.email which is used the src code.
    class AttrDict(dict):
    ...
    def mock_delete_user(user):
        return True
    monkeypatch.setattr(src.api.users.users, "get_user_by_id", mock_get_user_by_id)
    monkeypatch.setattr(src.api.users.users, "delete_user", mock_delete_user)
    client = test_app.test_client()
